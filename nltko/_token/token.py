...
class Token:
    ...
    regex_token  = r'[0-9A-Za-zㄱ-ㅎ가-힣]+' # r'[A-z0-9ㄱ-힣\.\%\,]+' # 
    regex_token_str = r'[A-z가-힣]+' # r'[A-z0-9ㄱ-힣\.\%\,]+' # 
    ...
    def _tokenizer(self, texts:list, bigrams=False, flat=True, only_text=True) -> list:
        r"""분석을 위한 한글 token 
        texts : List[`sentence`]
        flat  : 1차원 평평화 작업 적용
        bigrams : bigram 으로 Tokenizer 적용 
        only_text : tokenizer 단어만 추출 cf)1월, 15일 같이 숫자가 포함된 문자는 제외 """
        ...
        if only_text:
            _lambda = lambda x :" ".join(re.findall(self.regex_token_str, x))
        else:
    ...
    def split_token_kor(tokens:list):
        r"""한글로 구성된 Token, 영문 또는 숫자가 포함된 Token 구분하기"""

        assert type(tokens) == list, f"{type(tokens)} only allowed list"
        _lambda    = lambda x : len(re.findall(r'[ㄱ-ㅎA-z]+', x)) == 0 # 한글로만 구성된 단어
        tokens_kor = list(filter(_lambda, tqdm(tokens)))
        _lambda    = lambda x : len(re.findall(r'[A-z]+', x)) > 0 # 외국어가 포함단어 필터
        tokens_eng = list(filter(_lambda, tqdm(tokens)))
        content = {}
        content['kor'] = tokens_kor
        content['withEng'] = tokens_eng
        return content
